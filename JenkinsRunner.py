...
def run_job_details(self):
    ...
    if job_name == 'create_file':
        helper.create_file_in_workspace(self, file_name, 'This file is created during the build job.')
    elif job_name == 'read_file':
        helper.read_file_in_workspace(self, "file_name")
    elif job_name == 'free_disk':
        helper.print_free_disk_space()
    elif job_name == 'move_text':
        helper.move_text(self, source_file, destination_directory)
    elif job_name == 'schedule':
        helper.schedule_job(self, job_name, "0 8 * * *")
    else:
